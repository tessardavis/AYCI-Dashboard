# Cohort Dashboard — a guide

A plain-English walkthrough of what the **Cohort Dashboard** shows and how to use it.

The dashboard gives you a live health-check of a single cohort: how many people are in it, whether they've joined the Circle community, how engaged they are, and who still needs chasing. It pulls live from the **Monday Academy Members board**, cross-referenced with **Circle** membership and **Kit (ConvertKit)** tags.

> **Pick the cohort** using the dropdown at the top right (e.g. "June 26"). Everything on the page updates to that cohort. Hit **Refresh** if you want the very latest numbers.
...
A breakdown of the cohort by **membership tier** (Academy, VIP, Private Plus, etc.) — counts and percentages. Tells you the make-up of the cohort at a glance (e.g. how many are private-tier and need closer support).

---

## Milestones completed

The **5 engagement milestones** tracked on Circle (the same ones in the Student Lookup engagement bar). For each milestone it shows how many cohort members have reached it. Low early numbers are normal — they climb as the cohort works through the programme. Useful for spotting whether the group is progressing.

---

## Circle community join rate

The same story as the "On Circle" card, in detail:
...
How many cohort members have **posted in the "Introduce Yourself" space** (each cohort has its own). A simple proxy for early engagement — people who introduce themselves tend to stay active. The percentage bar shows adoption.
...
- **Numbers are live but cached briefly** — if you've just changed something, give it a moment or hit **Refresh**.
- **New vs Legacy is driven by Kit tags**, joined/tagged status by **Circle**, and the roster by **Monday** — so a person showing oddly is almost always a mismatch between those (e.g. a different signup email). Flag those to Tessa.
- **Each cohort is configured separately** (its Circle tag, Kit tags and intros space). If a brand-new cohort looks empty or wrong, its config may not be set up yet — that's a Settings task for an admin, not a bug to worry about.

---

*Questions about a specific number or student? Ask Tessa — most surprises come down to an email/tag mismatch between Monday, Kit and Circle.*
