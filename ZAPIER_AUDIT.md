...
| 16 | `8g: 15 minute call booked - Tessa` → **RENAME to Becky + migrate** | **Calendly: Invitee Created** | Same as 15 | The zap is mislabeled "Tessa" — it's actually **Becky's** (Tessa doesn't do these calls). Rename to Becky and migrate same as 15 (`fifteen_minute_call: "Booked"`). |
| 16b | `8g: 15 minute call booked - Anoop` | **Calendly: Invitee Created** | Same as 15 | Anoop is the third coach doing 15-min calls. **Migrate** same as 15 (`fifteen_minute_call: "Booked"`). |
...
- `8g: 15 minute call booked` — three coaches do these: **Charlotte, Anoop, Becky** (NOT Tessa). All write the same payload (`fifteen_minute_call: "Booked"` — the column has only Eligible/Booked, no per-coach labels). Charlotte **✅ migrated 2026-06-04**. The zap labelled "Tessa" is actually **Becky's** → rename + migrate. Anoop → migrate.
