...
|---|---|---|---|---|
| 1 | `[AYCI JUNE-26] - Signups to Monday Board (OD)` 🟦 **CREATE PATHS PHASE-1 MIGRATED 2026-06-07** | **Kajabi: New Purchase** | 1. Zapier Tables: Find Records (cohort lookup table) <br> 2. Filter (continue only for AYCI JUNE-26 offers) <br> 3. Formatter: Cohort Start Date <br> 4. Formatter: Cohort End Date <br> 5. Paths split by tier: Academy / Academy Private Plus / Academy VIP / Legacy Upgrade to Private Plus / Legacy Upgrade to Academy <br> 6. **New tier paths**: Create Item → Delay → Add email to trigger Tally onboarding form <br> 7. **Legacy upgrade paths**: Get Items by Column Value → **Update Item** | **THE P0 - done (Phase 1).** The 3 **create** paths (Academy/PP/VIP) now each POST to `/api/students-db/intake` right after Create Item → new students land in the dashboard instantly (email/name from Kajabi trigger, `tier` literal per path, `cohort_joined: "June 26"` literal, cohort_start/end from the step 4/5 formatters, `source: kajabi`). Create Items left in place as safety net; mirror reconciliation merges the auto: row. **Per-launch:** the `cohort_joined` literal must be updated each cohort (see memory). **Legacy-upgrade paths NOT migrated** - they update existing students (~8 fields incl. unmodeled Tier End Date / In Active Cohort / Legacy); mirror keeps them synced, so deferred to the remove-Monday pass. **Remove the Create Items only after ~1wk of verified signups.** |
...
| 15 | `8g: 15 minute call booked - Charlotte` ✅ **PHASE 2 DONE - Monday removed 2026-06-19** | **Calendly: Invitee Created** | monday Get Items by Column Value → **monday Update Item** → Slack | Re-pointed to `update-by-email` writing `fifteen_minute_call: "Booked"`. Monday steps left in place ~1wk as safety net, then remove. |
| 16 | `8g: 15 minute call booked - Becky` ✅ **PHASE 2 DONE 2026-06-19** | **Calendly: Invitee Created** | Same as 15 | Was mislabeled "Tessa" - actually Becky's. Renamed + re-pointed to `update-by-email` (`fifteen_minute_call: "Booked"`). |
| 16b | `8g: 15 minute call booked - Anoop` ✅ **PHASE 2 DONE 2026-06-19** | **Calendly: Invitee Created** | Same as 15 | Re-pointed to `update-by-email` (`fifteen_minute_call: "Booked"`). Calendly trigger test skipped (no recent bookings to sample - works on live runs). |
...
| 18 | `[AYCI JUNE-26] 1:1 Calls - Round Robin (Anoop)` ✅ **PHASE 2 DONE - Monday removed 2026-06-19** | **Calendly: Invitee Created** | 1. monday: Get Items by Column Value (look up student) <br> 2. monday: Get Column Values <br> 3. AI by Zapier × 2 (decide call number?) <br> 4. Paths (Call 1 / Call 2 / Call 3 / Call 4 / Fallback) <br> 5. Each call path: **monday: Update Item** (mark which 1:1 call number this is) <br> 6. Fallback: Formatter Date/Time → Circle Find Member → Slack alert | Round-robin 1:1 call booking. AI step picked which of 4 call slots to mark - **replaced by `book-call`** which does the slot logic server-side. **Phase 1:** added a Webhooks POST → `/api/students-db/book-call` `{email, coach:"Anoop"}` right after the trigger; left all Monday/AI/Paths steps in place as safety net (book-call uses the same lowest-un-booked rule + reads the Monday-mirrored slot state, so dashboard & Monday stay in sync). **Phase 2 (after ~1wk verify):** delete the 2 Monday read steps + 2 AI steps + 4 Call paths; replace the Paths block with one Filter "continue only if `slot` empty" → keep Fallback (Formatter → Circle → Slack). Three sibling zaps: Anoop / Charlotte / Becky. |
| 18b | `[AYCI JUNE-26] 1:1 Calls - Round Robin (Charlotte)` ✅ **PHASE 2 DONE 2026-06-19** | Same as 18 | Same (`coach:"Charlotte"`) | Sibling of 18. |
| 18c | `[AYCI JUNE-26] 1:1 Calls - Round Robin (Becky)` ✅ **PHASE 2 DONE 2026-06-19** | Same as 18 | Same (`coach:"Becky"`) | Sibling of 18. |
...
| 69 | `8c. Substantive success form - Add Boss Tag on Monday` (v5) 🟦 **PHASE 1 DONE 2026-06-05** | **monday: Specific Column Value Changed** → Filter → Delay After Queue → **monday: Get Items by Column Value** → Paths (A / B) → each path **monday: Update Item**. Path A (trigger Item Id Exists) writes `Boss Badge = Yes`; Path B (step-4 lookup Does Not Exist) is the not-found fallback (no badge write). **Phase 1:** added Webhooks POST → `update-by-email` `{email, boss_badge:"Yes"}` in Path A (email mapped from trigger's main Email col `email_mkqxv0j0`); Monday Update Item left as safety net. `boss_badge` added to PROTECTED_FIELDS. **Full retire still blocked on the Monday trigger** (needs outbound webhook / task #33). Path B untouched. |
| 70 | `8b. Substantive success form - tags Boss on Circle, CK and give bonus content access` (v7) 🟦 **MIGRATION IN PROGRESS 2026-06-05** | ~~monday: Specific Column Value Changed~~ → now **Webhooks Catch Hook** subscribed to `boss_badge` (first outbound-dispatcher consumer; proved the dispatcher end-to-end) → Filter `Changed Boss Badge = Yes` → Delay → Circle Find Member (Student Circle Email) → Circle Tag Member (Boss) → Kit Add Tag (Student Email) → Circle Add Member to Space → Circle Send DM. **Hard cutover** (no parallel - it DMs the student). **PARKED until ~week of 2026-06-08:** the catch-hook draft is fully built (filter + all step mappings done), but publish is blocked on switching the 4 Circle steps' connection from **Oksana → Coralie**, and Coralie's Circle account isn't connected yet (she's back from leave next week - see memory). **Left as an unpublished draft;** the live published 8b still runs on its Monday trigger meanwhile (no gap, no double-DM - dashboard `boss_badge` events just hit the draft catch-hook harmlessly). Chain once published: 8c writes `boss_badge` via dashboard → dispatcher emits → 8b catches. |
...
- `POST /api/students-db/intake` - **LIVE 2026-06-07**: the AYCI signup zap's 3 create paths (new students) + the Kajabi add-on capture. Upserts by email; mirror reconciles auto: rows. (Legacy-upgrade + AYGI/waitlist signup paths still to migrate.)
- Outbound webhook dispatcher - fires `column_changed` events for any dashboard-originated write (coach edit / migrated zap) to subscribers of that column. **Admin UI added 2026-06-05** at `/webhooks` (board: students) to create/list/delete subscriptions; backend endpoints moved to `/api/webhook-subscriptions` (the old `/students-db/webhook-subscriptions` path was shadowed by `/students-db/{id}`). 0 live subscribers yet. **Next: wire 8b** to catch the `boss_badge` event (8c already writes boss_badge via the dashboard → emits on change).
- **Mirror-emit bridge - BUILT 2026-06-19.** Closes the caveat above: the 15-min Academy mirror now also emits `column_changed` for **Monday-side** changes, so the ~12 zaps that trigger on a Monday "Specific Column Value Changed" (e.g. 32, 34, 36, 37, 43, 45, 46, 47, 53, 69's trigger) can move their **trigger** onto a Zapier Catch Hook even before coaches stop editing Monday. Mechanics (`academy_members_mirror.full_sync` + `webhooks_outbound.active_subscription_columns`): probes active subscription columns once per sync; if any, diffs those columns per row and emits when a non-pinned column's stored value changed (skips first-time inserts = new students). **Zero overhead when nothing is subscribed** (no projection widening, no diff). Pinned/dashboard-owned fields are skipped (their Monday value is ignored anyway). **To migrate a Monday-trigger zap now:** create a subscription for its column at `/webhooks`, point the zap's trigger at the catch-hook URL, keep the Monday trigger ~1 wk as safety net, then remove.
...
- `8g: 15 minute call booked` ✅ **ALL MIGRATED** - Charlotte (2026-06-04), Becky + Anoop (2026-06-05). Three coaches do these: Charlotte, Anoop, Becky (NOT Tessa; the "Tessa" zap was mislabeled Becky's). All write `fifteen_minute_call: "Booked"`. Monday steps left in place ~1wk as safety net, then remove.
- `8c. Substantive success form - Add Boss Tag` 🟦 **PHASE 1 DONE 2026-06-05** - Path A writes `boss_badge: "Yes"` via `update-by-email`; Monday step left as safety net. Monday-triggered, so full retire still pending the outbound-webhook work. Path B is a not-found fallback (left untouched).
- 3 × `1:1 Round Robin` (Anoop, Charlotte, Becky) 🟦 **PHASE 1 DONE 2026-06-05** - `book-call` POST added after the trigger on all three; Monday/AI/Paths machinery left in place as safety net. Phase 2 teardown (delete Monday read steps + AI + Call paths, replace Paths with a single `slot`-empty Filter → Fallback) pending ~1wk verification.
  - RMW + AI step that picked which Call slot (1-4) to fill. **Built server-side 2026-06-04 (option B):** `POST /api/students-db/book-call` replaces the AI-by-Zapier step entirely.
