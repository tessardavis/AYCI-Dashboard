...
| 16 | ~~`8g: 15 minute call booked - Tessa`~~ **RETIRE — do not migrate** | **Calendly: Invitee Created** | Same as 15 | Tessa no longer does 15-min/debrief calls (only Charlotte + Anoop do). Turn this off, like the Tessa Mock Interview zap (19d). |
| 16b | `8g: 15 minute call booked - Anoop` (verify exists) | **Calendly: Invitee Created** | Same as 15 | Anoop is the second coach doing 15-min calls. **Migrate** same as 15 (`fifteen_minute_call: "Booked"`). Confirm this zap exists in Zapier — not seen in the original audit. |
...
- `8g: 15 minute call booked` — **Charlotte ✅ migrated 2026-06-04** (`update-by-email`, `fifteen_minute_call: "Booked"`). **Tessa = RETIRE** (she's not doing 15-min/debrief calls — only Charlotte + Anoop are). **Anoop = migrate** (confirm the zap exists; not in original audit).
