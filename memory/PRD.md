...
- **Acknowledge** (`POST /api/coach-activity/over-allowance/ack` + `over_allowance_acks` collection): one-click button on each widget row records the current `over_by` and immediately removes the row from the cached snapshot. Row re-appears only if the student goes further over (acked at +1, returns at +2). Snapshot filter is also applied in `find_over_allowance_students` so subsequent Slack DMs respect the ack.
- **UI surfaces**:
  - "Over-allowance bookings" panel on `/coach-activity` (`OverAllowanceWidget.jsx`) — tier badge, breakdown of allowance, `+N over` chip, Acknowledge button, link to Monday row.
  - Inline `+N over allowance` chip on `/interviews` student cards (Academy + Private), tooltip explains Slack notification.
- **Endpoints**: `GET /api/coach-activity/over-allowance` (cached snapshot, `?refresh=true` to recompute), `POST .../notify` (force Slack cycle), `POST .../ack` (acknowledge).
- **Verified live**: 5 students flagged (Nalaayeni +8, Fiona/Amal/Laura/Rajdip +1). Ack flow tested end-to-end — clicking Fiona's button dropped her row instantly while leaving the other 4. Other 4 still flagged.
...
- **Label-only mode (new semantics on `ingest_inbound`)**: previously, inboxes flagged `ingest_inbound: false` were skipped entirely, which silently broke label routing for "personal-inbox-but-please-only-pull-tagged-threads" use cases. New behaviour:
  - `ingest_inbound: true` → pull every inbound email (support-desk style)
...
- **Deploy required** for the auto-routing to take effect on production.
